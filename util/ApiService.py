...
from util.HelperFunctions import get_current_month_info, mask_sensitive
...
class ApiService:
        ...
        try:
            ...
            if ("token失效" in rsp.get("msg", "未知错误")
                ...
                if self.login():
                    ...
                    raise ValueError("Token失效且重新登录失败")
            else:
    ...
    def login(self) -> bool:
        ...
        try:
            ...
            logger.info(f"登录请求已发送（账号: {mask_sensitive(ConfigManager.get('user', 'phone'))})")
            ...
            logger.info(f"登录结果：{mask_sensitive(user_info)}")
    ...
    def submit_clock_in(self, checkin_info: Dict[str, Any], config_key: str = "clockIn") -> dict[str, dict[str, Any] | bool] | None:
        """
        提交打卡信息。
        ...
        Args:
            ...
            config_key (str): 配置键，"clockIn"（上班）或 "clockInOff"（下班）。
        ...
        if UserInfoManager.get("userType") != "teacher":
            ...
            sign_data = [
                ...
                planId,
                UserInfoManager.get_userid(),
                ConfigManager.get(config_key, "location", "address")
            ]
        ...
        data.update(ConfigManager.get(config_key, "location"))
        ...
        if responses.get("msg") == "302":
            ...
            return {"result": rsp.get("code") == 200, "data": rsp}
        else:
            return {"result": responses.get("code") == 200, "data": responses}
