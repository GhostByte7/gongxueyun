...
def clock_in() -> dict[str, str]:
    ...
    # 根据打卡类型选择对应的配置 key
    config_key = "clockIn" if checkin_type == "START" else "clockInOff"
    ...
    success = api_client.submit_clock_in(checkin_info, config_key)

    # 记录获取结果
    if success.get("result"):
        logger.info("打卡成功")
        content = f"签到账号：{ConfigManager.get('user', 'phone')}\n签到地点：{ConfigManager.get(config_key, 'location', 'address')}"
